...
class LoginWithGoogle(APIView):
    def post(self, request):
        ...
        if user_type == 'olduser':
            ...
            # Check if we're dealing with Azure domains or similar subdomains
            if 'canadacentral-01.azurewebsites.net' in current_domain:
                # For Azure subdomains, use the common root domain with leading dot
                cookie_domain = '.canadacentral-01.azurewebsites.net'
                logger.info(f"Using Azure domain configuration: {cookie_domain}")
            else:
                # For same domain or local development, don't set domain or use None
                cookie_domain = None
                logger.info("Using same-domain configuration (domain=None)")
            
            # Set cookies with conditional domain configuration
            cookie_kwargs = {
                'httponly': True,
            ...
            refresh_token_kwargs = cookie_kwargs.copy()
            refresh_token_kwargs.update({
                'value': tokens['refresh'],
                'max_age': 60 * 60 * 24 * 7,  # 1 week
            })
            if cookie_domain:
                refresh_token_kwargs['domain'] = cookie_domain
            ...
            logger.info(f"Cookies set with domain: {cookie_domain}")
            logger.info(f"Backend domain: {current_domain}")
