...
#     try:
#         logger.info(f" PendingUser: {user}")
...
#                 )
#             else:
#                 logger.info(f"Creating UserTree with parentid from initiator_id: {user.initiator_id}")
#                 # Fetch parent, ensuring that a 0 or None initiator_id results in a null parent
#                 parent = UserTree.objects.filter(id=user.initiator_id).first()

#                 # Create UserTree entry with a nullable parentid field
#                 UserTree.objects.create(
#                     id=generated_id,
#                     name=user.first_name if len(f"{user.first_name} {user.last_name}") > 15 
#                         else f"{user.first_name} {user.last_name}".strip(),
#                     profilepic=user.profile_picture,
#                     parentid=parent,  # This will be None if initiator_id was 0 or None
#                     event_choice=user.event_type,
#                     event_id=user.event_id
#                 )

#             # Update metrics and delete pending user
